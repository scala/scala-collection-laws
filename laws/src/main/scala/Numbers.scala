...
final class Numbers(n0: Int, nn0: Int, m0: Int, mm0: Int, r0: Int) {
  ...
  /** A value that can be used to index a collection `x` (-1 if the collection is empty) */
  def n: Int  = { used(0) = true; values.n }

  /** A positive integer */
  def nn: Int = { used(1) = true; values.nn }

  /** A value that can be used to index a collection `y` (-1 if the collection is empty) */
...
object Numbers {
  /** The underlying, unchecked values whose usage is also not monitored */
  final case class Values(n: Int, nn: Int, m: Int, mm: Int, r: Int) {}

  /** The equivalent to "empty"--the most boring set of numbers */
  ...
  /** Directly creates an instance of `Numbers`.  You should probably use a `Restricted` instead
    * to limit the values of `n` and `m` by the collection instance.
    */
  def apply(n: Int, nn: Int, m: Int, mm: Int, r: Int): Numbers = new Numbers(n, nn, m, mm, r)

  /** Explores number parameter space but restricts based upon the size of the
    * `x` and `y` collections in use.
    */
